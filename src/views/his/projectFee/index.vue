<template>
  <div class="app-container">
    ...
    <!-- 添加或修改岗位对话框 -->
    <!--千万看清楚！！这里是form  复制粘贴过来的都是queryParams！！所以一直报错是空的！！！！-->
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="关键字" prop="keyword">
          <el-input v-model="form.keyword" placeholder="请输入关键字名称" />
        </el-form-item>
        <el-form-item label="项目单价" prop="projectUnitprice">
          <el-input v-model="form.projectUnitprice" placeholder="请输入项目单价" />
        </el-form-item>
...
<script>
import { listPost, getPost, delPost, addPost, updatePost, exportPost } from "@/api/his/projectFee";
...
export default {
  ...
  //这里才是真正的从数据库查询数据getDicts所作的就是去数据库查询一堆的状态
  //要用的时候直接可以this.调用想要的路径即可statusOptions是一个数组
  created() {
    this.getList();//查询所有数据
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      /* const postId = row.projectId || this.ids
    ...
    submitForm: function() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          ...
          } else if(this.signn==1){
            console.log(this.form.projectName+"abc")
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const postIds = row.projectId || this.ids;
      this.$confirm('是否确认删除费用查询编号为"' + postIds + '"的数据项?', "警告", {
